...
if(isset($_POST['add'])){
    ...
    $chk = $conn->query("SELECT * FROM upload_toda WHERE name = '$name' ")->rowCount();
    ...
    if($chk){
        ...
        while($c == 0){
            ...
            $chk2 = $conn->query("SELECT * FROM upload_toda WHERE name = '$tname' ")->rowCount();
            if($chk2 == 0){
                $c = 1;
                $name = $tname;
            }
    ...
    $move = move_uploaded_file($temp,"toda/".$fname);

    if($move){
        // Include reference_number and date_time in the insert query
        $query = $conn->query("INSERT INTO upload_toda(name, fname, reference_number, date_time ,`proponent`, `seconded`, `committe`) VALUES('$name','$fname','$referenceNumber','$dateTime' , '$proponent', '$seconded', '$committe')");
        if($query){
            header("location: TODA.php");
            exit;
        } else {
            die(mysql_error());
        }
    ...
    <div class="span12">
        <div class="container">
            ...
            <input  type="text" class="form-control" id="searchInput" placeholder="Type to search..." oninput="filterTable()">
                ...
                <H4 style="position: static; left: 30px;">TODA Dashboard</H4>
        ...
        <div class="table-responsive">
            <table class="table table-hover table-striped table-bordered">
                ...
                <?php
                $query = $conn->query("SELECT * FROM upload_toda ORDER BY id DESC");
                while($row = $query->fetch()){
                    echo "<tr>";
                   
                    echo '<td style="text-align: center"><a href="toda_pdf.php?filename=' . $row['id'] . '&f=' . $row['fname'] . '">' . $row['name'] . '</a></td>';
