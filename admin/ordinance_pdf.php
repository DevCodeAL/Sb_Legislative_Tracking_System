...
if (isset($_POST['reset'])) {
    ...
    if ($file_id > 0) {
        $sql = "UPDATE upload_ord SET view_count = 0 WHERE id = ?";
...
if ($id > 0) {
    ...
    $stmt = $conn->prepare("SELECT * FROM upload_ord WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
...
function docToPdf($docFilePath) {
    ...
    $pdfFilePath = 'upload_ord/' . time() . '.pdf';
...
$file_path = 'upload_ord/' . $_REQUEST['f'];
