...
if(isset($_POST['add'])){
    ...
    if($move){
        ...
        $query = $conn->query("INSERT INTO upload_ord(name, fname, reference_number, date_time ,`proponent`, `seconded`, `committe`) VALUES('$name','$fname','$referenceNumber','$dateTime' , '$proponet', '$seconded', '$committe')");
        if($query){
...
if (isset($_POST['update'])) {
    ...
    $email = mysqli_real_escape_string($conns, $_POST['email']);

    // Update data in the database
    $sqls = "UPDATE admin_db SET username='$username', password='$password', name='$name' , email='$email' WHERE id = 11";
...
?>
            ...
            <div class="myModal" id="Modal">
                <div class="container-modals">
                    ...
                    <div class="w-50 p-3">
                        <label for="ReferenceNum">Resolution Number</label>
                        <input type="text" name="ReferenceNum" class="form-control mb-3" required>
                    </div>

                    <div class="w-50 p-3">
                    <label for="proponent">Proponent</label>
                    <input type="text" name="proponent" class="form-control mb-3" required>
                    </div>


                    <div class="w-50 p-3">
                        <label for="seconded">Seconded</label>
                        <input type="text" name="seconded" class="form-control mb-3" required>
                    </div>

                    <div class="w-50 p-3">
                        <label for="committe">Committe</label>
                        <input type="text" name="committe" class="form-control mb-3" required>
                    </div>
                    
                    <div class="w-50 p-3">
                    <label for="time">Date</label>
                    <input type="text" id="time" class="form-control mb-3" name="time" required>
                    </div>
                    ...
                    <button type="submit" name="add" id="add" class="btn btn-primary">Upload</button>
                </form>
                </div>
            </div>
    ...
    <div class="container-modals">
        ...
        <form class="form" enctype="multipart/form-data" action="ord_edit.php" name="form" method="post">
            <div class="w-75 p-3">
    ...
    <div class="container">
        <a class="navbar-brand" href="ordinances_dash.php">SB-CUYAPO</a>
    ...
    <div class="span12">
        ...
        <div class="search">
            ...
            <input  type="text" class="form-control" id="searchInput" placeholder="Type to search..." oninput="filterTable()">
                ...
                <H4 style="position: static; left: 30px;">Ordinances Dashboard</H4>
            ...
            <div class="table-responsive">
                ...
                <thead>
                    <tr>
                        <th style="text-align: center">Resolution No.</th>
                        <th style="text-align: center">Title</th>
                        <th style="text-align: center">Proponent</th>
                        <th style="text-align: center">Seconded</th>
                ...
                <?php
                $query = $conn->query("SELECT * FROM upload_ord ORDER BY id DESC");
                while($row = $query->fetch()){
                    echo "<tr>";
                    echo "<td>".$row['reference_number']."</td>";
                    echo '<td style="text-align: center"><a href="ordinance_pdf.php?filename=' . $row['id'] . '&f=' . $row['fname'] . '">' . $row['name'] . '</a></td>';
                    echo "<td>".$row['proponent']."</td>";
                    echo "<td>".$row['seconded']."</td>";
                    echo "<td>".$row['committe']."</td>";
                    echo "<td>".$row['date_time']."</td>";
                    echo '<td><button class="btn btn-primary edit-btn-ord"  data-id-ord="' . $row['id'] . '"><i class="fa-solid fa-pen-to-square"></i></button></td>';
                    echo '<td><button class="btn btn-danger"><a class="link" href="delete_ord.php?id=' . $row['id'] . '"><i class="fa-solid fa-trash"></i></a></button></td>';
                    echo "</tr>";
                ?>
                <?php }?>
                </tbody>
...
<script>
    $(document).ready(function() {
        ...
        $('.edit-btn-ord').on('click', function() {
            const id = $(this).data('id-ord');
