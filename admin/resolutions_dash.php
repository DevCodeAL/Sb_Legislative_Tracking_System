...
if(isset($_POST['add'])){
    ...
    if($move){
        ...
        $query = $conn->query("INSERT INTO upload_res(name, fname, reference_number, date_time ,`proponent`, `seconded`, `committe`) VALUES('$name','$fname','$referenceNumber','$dateTime' , '$proponent', '$seconded', '$committe')");
        if($query){
            header("location: resolutions_dash.php");
            exit;
        } else {
            die(mysql_error());
        }
...
?>
            ...
            <div class="myModal" id="Modal">
                <div class="container-modals">
                    ...
                    <div class="w-50 p-3">
                    <label for="proponents">Proponent</label>
                    <input type="text" name="proponents" class="form-control mb-3" required>
                    </div>
    ...
    <div class="span12">
        <div class="container">
            ...
            <input  type="text" class="form-control" id="searchInput" placeholder="Type to search..." oninput="filterTable()">
                ...
                <H4 style="position: static; left: 30px;">Resolutions Dashboard</H4>
        ...
        <div class="table-responsive">
            <table class="table table-hover table-striped table-bordered">
                ...
                <?php
                $query = $conn->query("SELECT * FROM upload_res ORDER BY id DESC");
                while($row = $query->fetch()){
                    echo "<tr>";
                    echo "<td>".$row['reference_number']."</td>";
                    echo '<td style="text-align: center"><a href="resolution_pdf.php?filename=' . $row['id'] . '&f=' . $row['fname'] . '">' . $row['name'] . '</a></td>';
                    echo "<td>".$row['proponent']."</td>";
                    echo "<td>".$row['seconded']."</td>";
                    echo "<td>".$row['committe']."</td>";
                    echo "<td>".$row['date_time']."</td>";
                    echo '<td><button class="btn btn-primary edit-btn-res"  data-id-res="' . $row['id'] . '"><i class="fa-solid fa-pen-to-square"></i></button></td>';
                    echo '<td><button class="btn btn-danger"><a class="link" href="delete_res.php?id=' . $row['id'] . '"><i class="fa-solid fa-trash"></i></a></button></td>';
                    echo "</tr>";
                ?>
                <?php }?>
                </tbody>
