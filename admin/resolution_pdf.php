...
if (isset($_POST['reset'])) {
    ...
    if ($file_id > 0) {
        $sql = "UPDATE upload_res SET view_count = 0 WHERE id = ?";
...
if ($id > 0) {
    ...
    $stmt = $conn->prepare("SELECT * FROM upload_res WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
...
function docToPdf($docFilePath) {
    ...
    $pdfFilePath = 'upload_res/' . time() . '.pdf';
...
$file_path = 'upload_res/' . $_REQUEST['f'];
