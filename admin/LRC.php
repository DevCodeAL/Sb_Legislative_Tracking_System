...
if(isset($_POST['add'])){
    ...
    $chk = $conn->query("SELECT * FROM upload_lrc WHERE name = '$name' ")->rowCount();
    ...
    if($chk){
        ...
        while($c == 0){
            ...
            $chk2 = $conn->query("SELECT * FROM upload_lrc WHERE name = '$tname' ")->rowCount();
            if($chk2 == 0){
                $c = 1;
                $name = $tname;
            }
    ...
    $move = move_uploaded_file($temp,"lrc/".$fname);

    if($move){
        // Include reference_number and date_time in the insert query
        $query = $conn->query("INSERT INTO upload_lrc(name, fname, reference_number, date_time ,`proponent`, `seconded`, `committe`) VALUES('$name','$fname','$referenceNumber','$dateTime' , '$proponent', '$seconded', '$committe')");
        if($query){
            header("location: LRC.php");
            exit;
        } else {
            die(mysql_error());
        }
...
<head>
    ...
    <title>Land ReClassification</title>
</head>
<body>
    ...
    <div class="span12">
        <div class="container">
            ...
            <input  type="text" class="form-control" id="searchInput" placeholder="Type to search..." oninput="filterTable()">
                ...
                <H4 style="position: static; left: 30px;">LRC Dashboard</H4>
        ...
        <div class="table-responsive">
            <table class="table table-hover table-striped table-bordered">
                ...
                <?php
                $query = $conn->query("SELECT * FROM upload_lrc ORDER BY id DESC");
                while($row = $query->fetch()){
                    echo "<tr>";
                   
                    echo '<td style="text-align: center"><a href="lrc_pdf.php?filename=' . $row['id'] . '&f=' . $row['fname'] . '">' . $row['name'] . '</a></td>';
