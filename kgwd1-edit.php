...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update'])) {
    ...
    $contact_numbersss1 = mysqli_real_escape_string($conn, $_POST['contacts']);
    ...
    $sql = "UPDATE `kgwd1-db` SET
            fullname=?, gender=?, date_of_birth=?,
            citizenship=?, civil_status=?, term=?,
            position=?, address=?, contact_numbers=?,
            email=?, ec_name=?, ec_number=?
            WHERE id=0";

    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssssssss", $fullname, $gender, $birthdate, $citizenship, $civilstatus, $term, $position, $address, $contact_numbersss1, $email, $emergencyname, $emergencycontact);
